...
**Signatürk** — Turkish Sign Language (TİD) **recognition** and **animation** in the browser, backed by FastAPI.
...
<p align="center">
  <b>Real-time sign recognition from webcam + text-to-sign 3D stick figure animation</b>
</p>
...
MediaPipe Holistic runs entirely in the browser (WASM) and extracts hand + pose landmarks in real time. The 252-dimensional landmark vectors are streamed to the backend via **WebSocket** (`ws://` or `wss://` depending on the page protocol) where the BiLSTM model performs inference.
...
```
├── backend.py                    # FastAPI — WebSocket, ML inference, landmark API, GET /startup-check
...
| Endpoint | Purpose |
|----------|---------|
| `GET /health` | Backend up; `num_classes`, `seq_len` |
| `GET /startup-check` | Config/model files on disk, landmark index count |

The root HTML response uses **`Cache-Control: no-store`** so hard-refresh picks up frontend changes reliably.
...
- **Editor:** Local `.vscode/` settings are gitignored; use your own launch/debug config.
